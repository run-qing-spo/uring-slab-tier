#!/usr/bin/env python3
"""顺序运行M3（单SQE submit）的五个workload。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="运行完整uring-slab M3 suite")
    ...
    for name, workload_args in workloads:
        output = output_dir / f"m3-{name}.json"
        ...
        if not args.keep_data:
            shutil.rmtree(args.root_dir.resolve() / f"m3-{run_id}")
