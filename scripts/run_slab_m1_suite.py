#!/usr/bin/env python3
"""顺序运行M1（Python pool + 单slab）的五个workload。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="运行完整M1 suite")
    ...
    for name, workload_args in workloads:
        output = output_dir / f"m1-{name}.json"
        ...
        if not args.keep_data:
            shutil.rmtree(args.root_dir.resolve() / f"m1-{run_id}")
