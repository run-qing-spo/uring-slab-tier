#!/usr/bin/env python3
"""顺序运行M2（C++ blocking pool）的五个workload。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="运行完整uring-slab M2 suite")
    ...
    for name, workload_args in workloads:
        output = output_dir / f"m2-{name}.json"
        ...
        if not args.keep_data:
            shutil.rmtree(args.root_dir.resolve() / f"m2-{run_id}")
